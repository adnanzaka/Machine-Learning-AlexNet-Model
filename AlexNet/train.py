...
def train(
    ...
    with tf.device("/cpu:0"):
        q = tf.FIFOQueue(
            batch_size * 3, [tf.float32, tf.float32], shapes=[[224, 224, 3], [1000]]
        )
    ...
    with tf.name_scope("cross_entropy"):
        cross_entropy = tf.reduce_mean(
            tf.nn.softmax_cross_entropy_with_logits(
                logits=pred, labels=y_b, name="cross-entropy"
            )
        )

    with tf.name_scope("l2_loss"):
        l2_loss = tf.reduce_sum(
            lmbda * tf.stack([tf.nn.l2_loss(v) for v in tf.get_collection("weights")])
        )
        ...
        for e in range(sess.run(epoch), epochs):
            for i in range(num_batches):
                ...
                summary_str, _, step = sess.run(
                    ...
                    feed_dict={lr: learning_rate, keep_prob: dropout},
                )
                ...
                if step % display_step == 0:
                    c, a = sess.run(
                        [loss, accuracy], feed_dict={lr: learning_rate, keep_prob: 1.0}
                    )
                    print(
                        "Epoch: {:03d} Step/Batch: {:09d} --- Loss: {:.7f} Training accuracy: {:.4f}".format(
                            e, step, c, a
                        )
                    )
                ...
                if step % test_step == 0:
                    val_im, val_cls = tu.read_validation_batch(
                        ...
                        os.path.join(
                            imagenet_path, "data/ILSVRC2012_validation_ground_truth.txt"
                        ),
                    )
                    v_a = sess.run(
                        accuracy,
                    ...
                    int_time = time.time()
                    print(
                        "Elapsed time: {}".format(tu.format_time(int_time - start_time))
                    )
