...
def test(top_k, k_patches, display_step, imagenet_path, ckpt_path):
    ...
    test_labels = tu.read_test_labels(
        os.path.join(imagenet_path, "data/ILSVRC2012_validation_ground_truth.txt")
    )
    ...
    with tf.Session(config=tf.ConfigProto()) as sess:
        ...
        total_topk_accuracy = 0.0

        for i in range(test_examples):
            # taking a few patches from an image
            image_patches = tu.read_k_patches(
                os.path.join(imagenet_path, "ILSVRC2012_img_val", test_images[i]),
                k_patches,
            )
            label = test_labels[i]

            top1_a, topk_a = sess.run(
                [top1_accuracy, topk_accuracy], feed_dict={x: image_patches, y: [label]}
            )
            ...
            if i % display_step == 0:
                print(
                    "Examples done: {:5d}/{} ---- Top-1: {:.4f} -- Top-{}: {:.4f}".format(
                        i + 1,
                        test_examples,
                        total_top1_accuracy / (i + 1),
                        top_k,
        ...
        print(
            "Top-1: {:.4f} -- Top-{}: {:.4f}".format(
                total_top1_accuracy / test_examples,
                top_k,
                total_topk_accuracy / test_examples,
            )
        )
        print(
            "Top-1 error rate: {:.4f} -- Top-{} error rate: {:.4f}".format(
                1 - (total_top1_accuracy / test_examples),
                top_k,
...
if __name__ == "__main__":
    ...
    test(TOP_K, K_PATCHES, DISPLAY_STEP, IMAGENET_PATH, CKPT_PATH)
